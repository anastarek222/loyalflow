...
test("TC4.19 preserves card permissions and composes with later Blob lifecycle guards", () => {
  ...
  assert.match(customLifecycle, /canPerformSubscriptionOperation\(/);
  assert.match(customLifecycle, /canBusinessPerformSubscriptionOperation\(/);
});
