...
test("TC6.12 bulk status and tag commands commit durable Sheets jobs in their transactions", () => {
  ...
  assert.match(command, /integrationJobId: integrationJob\.id/g);
  assert.match(command, /changedIds\.length === 0/);
});

test("TC6.12 bulk action wakes durable jobs while Customer creation stays on its separate existing path", () => {
  ...
  assert.doesNotMatch(bulkSource, /await syncBusinessToGoogleSheetSafely/);
  assert.match(createSource, /await syncBusinessToGoogleSheetSafely\(business\.id\)/);
});
