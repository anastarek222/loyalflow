...
test("U9 retains canonical report routes and slug-preserving shared navigation", () => {
  ...
  assert.match(
    source("components/reports/report-navigation.tsx"),
    /\/businesses\/\$\{slug\}\/reports/,
  );
...
test("U9 validates bounded inclusive dates and tenant-scoped branch/staff filters", () => {
  ...
  assert.equal(
    parseReportDateRange({ from: "2026-02-31", to: "2026-03-01" }),
    null,
  );
  assert.equal(
    parseReportDateRange({ from: "2026-01-01", to: "2027-01-02" }),
    null,
  );
  assert.equal(
    resolveReportScope({
      businessId: "a",
      branches: [
        { id: "foreign", businessId: "b", name: "Foreign", isActive: true },
      ],
      staff: [],
      branchId: "foreign",
    }),
...
test("U9 preserves supported filters for export and protects export server-side", () => {
  const query = getReportQueryString({
    ...
    loyaltyMode: "POINTS",
    branchId: "branch-a",
    attributedStaffId: "staff-a",
  });
  assert.match(query, /segment=ACTIVE/);
  assert.match(query, /branch=branch-a/);
  assert.match(query, /staff=staff-a/);
...
test("U9 uses persisted attribution, safe chart values, localized responsive foundations", () => {
  ...
  assert.match(
    source("components/reports/report-charts.tsx"),
    /safeReportNumber/,
  );
  assert.equal(safeReportNumber(Number.NaN), 0);
  assert.equal(safeReportNumber(Infinity), 0);
  const chart = source("components/reports/report-charts.tsx");
  assert.match(chart, /ResponsiveContainer/);
  assert.match(chart, /language === "AR"/);
  ...
  assert.match(
    source("app/businesses/[slug]/reports/loading.tsx"),
    /aria-busy/,
  );
});
...
test("U9 never labels visits or points as currency, while recorded sales remain explicit", () => {
  assert.match(
    source("lib/analytics/report-filters.ts"),
    /saleAmount: \{ not: null \}/,
  );
