...
test("TC4.17 keeps adjacent higher-risk settings actions deferred", () => {
  ...
  for (const deferred of [syncAction, exportAction]) {
    assert.doesNotMatch(deferred, /canPerformSubscriptionOperation\(/);
    assert.doesNotMatch(deferred, /canBusinessPerformSubscriptionOperation\(/);
  }
});
