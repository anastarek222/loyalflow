...
test.describe.serial("PR browser smoke", () => {
  ...
  test("Customer detail SIMPLE preserves loyalty, timeline and card composition @desktop @pr-smoke", async ({
    page,
  }) => {
    await signIn(page, "owner-a");
    const route = `/businesses/${fixture.businessA}/customers/${fixture.activeCustomer.id}`;
    ...
    for (const viewport of [
      ...
      for (const locale of ["en", "ar"] as const) {
        ...
        for (const theme of ["light", "dark"] as const) {
          ...
          const hero = main.locator("[data-customer-profile-hero]");
          await expect(hero).toBeVisible();
          await expect(hero.locator("h1")).toHaveText("Final UAT active");

          await expect(main.locator("#daily-loyalty")).toBeVisible();
          await expect(
            main.locator("[data-customer-activity-timeline]"),
          ).toBeVisible();
          const customerCard = main.locator("#customer-card");
          await expect(customerCard).toBeVisible();
          await expect(main.locator("#customer-details")).toBeHidden();
          ...
          const cardBox = await customerCard.boundingBox();
          expect(heroBox).not.toBeNull();
          expect(cardBox).not.toBeNull();
          if (viewport.width >= 1280) {
            expect(Math.abs(heroBox!.y - cardBox!.y)).toBeLessThan(180);
          } else {
  ...
  test("Customer detail ADVANCED preserves management disclosures and responsive composition @desktop @pr-smoke", async ({
    page,
  }) => {
    await signIn(page, "manager-a");
    const route = `/businesses/${fixture.businessA}/customers/${fixture.activeCustomer.id}`;
    ...
    for (const viewport of [
      ...
      for (const locale of ["en", "ar"] as const) {
        ...
        for (const theme of ["light", "dark"] as const) {
          ...
          const customerDetails = main.locator("#customer-details");
          await expect(customerDetails).toBeVisible();
          await customerDetails.locator("summary").click();
          await expect(customerDetails.locator("#editFirstName")).toBeVisible();
          await expect(customerDetails.locator("#editPhone")).toBeVisible();

          const disclosures = main.locator("[data-operational-disclosure]:visible");
