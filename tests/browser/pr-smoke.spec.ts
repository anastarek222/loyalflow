...
test.describe.serial("PR browser smoke", () => {
  ...
  test("Customers SIMPLE preserves search and mobile-first list across locale theme and viewport @desktop @pr-smoke", async ({
    page,
  }) => {
    await signIn(page, "owner-a");
    const route = `/businesses/${fixture.businessA}/customers`;
      ...
      for (const locale of ["en", "ar"] as const) {
        ...
        for (const theme of ["light", "dark"] as const) {
          ...
          const customers = page.locator(
            'main[data-experience-customers="simple"]',
          );
          await expect(customers).toBeVisible();
          await expect(customers).toHaveAttribute(
            "data-experience-mode",
            "SIMPLE",
          );
          await expect(
            customers.getByRole("heading", {
              level: 1,
              name: locale === "ar" ? "العملاء" : "Customers",
              exact: true,
            }),
          ...
          await expect(
            customers.locator('input[name="q"]'),
          ).toHaveAttribute(
            "placeholder",
            locale === "ar"
              ? "الاسم أو الهاتف أو كود العميل"
          ...
          const mobileList = customers.getByLabel(
            locale === "ar"
              ? "قائمة العملاء على الجوال"
              : "Mobile customer list",
            { exact: true },
          );
          await expect(mobileList).toBeVisible();
          expect(
            await mobileList.locator(
              `a[href^="/businesses/${fixture.businessA}/customers/"]`,
            ).count(),
          ).toBeGreaterThan(0);

          const simpleAdvanced = customers
            .locator("details")
            .filter({
              hasText:
                locale === "ar" ? "خيارات متقدمة" : "Advanced options",
            })
            .last();
          await expect(simpleAdvanced).toBeVisible();
          ...
          await expect(
            page.locator("[data-app-language]").first(),
  ...
  test("Customers ADVANCED preserves filters and responsive table/list across locale theme and viewport @desktop @pr-smoke", async ({
    page,
  }) => {
    await signIn(page, "manager-a");
    const route = `/businesses/${fixture.businessA}/customers`;
    ...
    for (const viewport of [
      ...
      for (const locale of ["en", "ar"] as const) {
        ...
        for (const theme of ["light", "dark"] as const) {
          ...
          const customers = page.locator(
            'main[data-experience-customers="advanced"]',
          );
          await expect(customers).toBeVisible();
          await expect(customers).toHaveAttribute(
            "data-experience-mode",
          ...
          const advancedOptions = customers
            .locator("details")
            .filter({
              hasText:
                locale === "ar" ? "خيارات متقدمة" : "Advanced options",
          ...
          await expect(customers.locator('select[name="status"]')).toBeVisible();
          await expect(customers.locator('select[name="segment"]')).toBeVisible();
          await expect(customers.locator('select[name="sort"]')).toBeVisible();

          const desktopTable = customers.locator("table");
          const mobileList = customers.getByLabel(
            locale === "ar"
              ? "قائمة العملاء على الجوال"
              : "Mobile customer list",
