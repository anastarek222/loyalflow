    ...
    {!simple ? (
      <details className="rounded-[var(--lf-radius-card)] border border-border bg-surface">
        <summary className="flex min-h-14 cursor-pointer list-none items-center justify-between gap-4 px-5 py-4">
          <span>
            <span className="block text-sm font-bold text-foreground">{language === "AR" ? "إضافة مكافأة" : "Add reward"}</span>
            <span className="mt-0.5 block text-xs text-foreground-subtle">{language === "AR" ? "أنشئ مكافأة جديدة فقط عند الحاجة." : "Create a new reward only when you need one."}</span>
          </span>
          <span className="text-lg font-bold text-primary" aria-hidden="true">+</span>
        </summary>
        <form action={createReward} className="grid gap-3 border-t border-border p-5 sm:grid-cols-2">
          <Field name="name" label={language === "AR" ? "اسم المكافأة" : "Reward name"} required />
          <Field name="cost" type="number" label={`${language === "AR" ? "التكلفة" : "Cost"} (${business.unitName})`} required />
          <Field name="expiresAfterDays" type="number" label={language === "AR" ? "أيام الصلاحية (اختياري)" : "Expiry days (optional)"} />
          <label className="text-sm font-semibold text-foreground-muted">{language === "AR" ? "النوع" : "Type"}<select name="type" defaultValue="GIFT" className="mt-1 min-h-11 w-full rounded-[var(--lf-radius-input)] border border-border bg-surface px-3"><option value="GIFT">Gift</option><option value="PROMO_CODE">Promo code</option><option value="DISCOUNT">Discount</option><option value="CUSTOM">Custom</option></select></label>
          <Field name="code" label={language === "AR" ? "الكود (اختياري)" : "Code (optional)"} />
          <label className="text-sm font-semibold text-foreground-muted">{language === "AR" ? "الوصف" : "Description"}<textarea name="description" rows={3} maxLength={300} className="mt-1 w-full rounded-[var(--lf-radius-input)] border border-border bg-surface p-3" /></label>
          <button type="submit" className="min-h-11 rounded-[var(--lf-radius-input)] bg-primary px-4 font-semibold text-white hover:bg-primary-hover sm:col-span-2">{language === "AR" ? "إضافة مكافأة" : "Add reward"}</button>
        </form>
      </details>
    ) : null}
    <section>
      <div className="space-y-4"><div className="grid grid-cols-3 overflow-hidden rounded-[var(--lf-radius-card)] border border-border bg-surface"><Summary label={language === "AR" ? "نشطة" : "Active"} value={business.rewards.filter((r) => r.isActive).length} /><Summary label={language === "AR" ? "إجمالي الكتالوج" : "Catalog total"} value={business.rewards.length} /><Summary label={language === "AR" ? "الخيار الاحتياطي" : "Fallback"} value={`${business.rewardThreshold} ${business.unitName}`} /></div>
...
function Summary({ label, value }: { label: string; value: string | number }) { return <div className="border-s border-border px-4 py-4 first:border-s-0"><p className="text-xs font-semibold text-foreground-subtle">{label}</p><p className="mt-1 text-xl font-black text-foreground">{value}</p></div>; }
