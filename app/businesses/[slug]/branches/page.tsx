...
export default async function BranchesPage({
  ...
  const totalAssignments = branches.reduce(
    (total, branch) => total + branch._count.staffAssignments,
    0,
  );
      ...
      <div className="mx-auto max-w-7xl" data-branches-administration="true">
          ...
          <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
            <div className="max-w-2xl">
              ...
              <p className="mt-2 text-sm leading-6 text-foreground-muted">
                أضف فروع النشاط وحدد موظفي التشغيل المخولين بالعمل في كل فرع.
              </p>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <BranchMetric label="الإجمالي" value={getBranchCount(branches)} />
              <BranchMetric label="النشطة" value={activeBranches} />
              <BranchMetric label="الإسنادات" value={totalAssignments} />
            </div>
          </div>
        </header>

        {query.success === "created" && (
          <Notice tone="success">تم إنشاء الفرع بنجاح.</Notice>
        )}
        {query.success === "updated" && (
          <Notice tone="success">تم تحديث بيانات الفرع.</Notice>
        )}
        {query.success === "activated" && (
          <Notice tone="success">تم تفعيل الفرع.</Notice>
        )}
        {query.success === "deactivated" && (
          <Notice tone="warning">
            تم إيقاف الفرع. لن تقبل عمليات الولاء الجديدة عليه.
          </Notice>
        )}
        {query.success === "assigned" && (
          <Notice tone="success">تم إسناد الموظف إلى الفرع.</Notice>
        )}
        {query.success === "assignment-removed" && (
          <Notice tone="success">تمت إزالة إسناد الموظف بأمان.</Notice>
        )}
        {query.error === "invalid" && (
          <Notice tone="error">راجع بيانات الفرع أو الاختيار المدخل.</Notice>
        )}
        {query.error === "duplicate-name" && (
          <Notice tone="error">يوجد فرع آخر بالاسم نفسه في هذا النشاط.</Notice>
        )}
        {query.error === "duplicate-assignment" && (
          <Notice tone="warning">هذا الموظف مسند إلى الفرع بالفعل.</Notice>
        )}
        {query.error === "ineligible-user" && (
          <Notice tone="error">يمكن إسناد موظف نشط من نفس النشاط فقط.</Notice>
        )}
        ...
        {query.error === "subscription-restricted" && (
          <Notice tone="warning">
            ...
            حالته أو إسنادات موظفيه. تظل الفروع والبيانات الحالية متاحة
            للقراءة.
          </Notice>
        )}
        ...
        <details className="group mb-6 rounded-[var(--lf-radius-card)] border border-border bg-surface shadow-sm">
            ...
            <span className="flex items-center gap-3">
              ...
              <span>
                ...
                <span className="mt-0.5 block text-xs text-foreground-subtle">
                  اسم الفرع مطلوب، وباقي البيانات اختيارية.
                </span>
              </span>
            </span>
            <span
          ...
          <form
            action={createBranch}
            className="grid gap-4 border-t border-border p-5 md:grid-cols-2 xl:grid-cols-4"
          >
            <input
              name="name"
              required
            ...
            <input
              name="contactPhone"
              maxLength={25}
              placeholder="هاتف التواصل (اختياري)"
              className={branchFieldClass}
            />
            <input
              name="address"
            ...
            <button
              type="submit"
              className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[var(--lf-radius-input)] bg-primary px-5 font-bold text-[var(--lf-primary-foreground)] transition-colors hover:bg-primary-hover"
            >
        ...
        <section className="space-y-4">
          {branches.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500">
              لا توجد فروع بعد. يبقى مسار الموقع الواحد الحالي مدعومًا حتى تضيف
              فرعًا.
            </div>
          ) : (
            branches.map((branch) => {
              const assignedUserIds = new Set(
                branch.staffAssignments.map((assignment) => assignment.user.id),
              );
              const availableStaff = eligibleStaff.filter(
                (user) => !assignedUserIds.has(user.id),
              );
              const updateBranch = updateBranchAction.bind(
                null,
                business.slug,
                branch.id,
              );
              const setBranchStatus = setBranchStatusAction.bind(
                null,
                business.slug,
                branch.id,
                !branch.isActive,
              );
              const assignStaff = assignStaffToBranchAction.bind(
                null,
                business.slug,
                branch.id,
              );
                  ...
                  <div className="flex flex-col gap-4 border-b border-slate-100 pb-5 sm:flex-row sm:items-start sm:justify-between">
                    <div className="flex min-w-0 items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          <span
                            className={`rounded-full px-3 py-1 text-xs font-bold ${branch.isActive ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-700"}`}
                          >
                            {branch.isActive ? "نشط" : "موقوف"}
                          </span>
                        </div>
                        <p className="mt-2 text-sm text-slate-500">
                          أُنشئ في {dateFormatter.format(branch.createdAt)} ·{" "}
                          {branch._count.staffAssignments} موظف مسند
                        </p>
                    ...
                    <form action={setBranchStatus}>
                      <ConfirmSubmitButton
                        confirmation={
                          branch.isActive
                            ? `إيقاف فرع ${branch.name}؟ لن يقبل عمليات ولاء أو إسنادات جديدة حتى إعادة تفعيله.`
                            : `تفعيل فرع ${branch.name}؟`
                        }
                        type="submit"
                    ...
                    <div className="border-t border-border p-4">
                      ...
                      >
                        <label className="text-sm font-bold text-slate-700">
                          ...
                          <input
                            ...
                            className={`${branchFieldClass} mt-2 font-normal`}
                          />
                        </label>
                        <label className="text-sm font-bold text-slate-700">
                          هاتف التواصل
                        ...
                        <button
                          type="submit"
                          className="rounded-xl border border-violet-200 bg-violet-50 px-5 py-3 font-bold text-violet-700 transition hover:bg-violet-100 md:col-start-3"
                        >
                          حفظ التعديلات
                        </button>
                      </form>
                      ...
                      <div className="mt-6 border-t border-slate-100 pt-5">
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                          تُسند حسابات الموظفين النشطة فقط؛ المديرون والمشاهدون
                          لا يحتاجون إسناد فرع.
                        </p>
                        {branch.staffAssignments.length > 0 && (
                          <ul className="mt-4 space-y-2">
                            {branch.staffAssignments.map((assignment) => {
                              const removeAssignment =
                                removeStaffAssignmentAction.bind(
                                  null,
                                  business.slug,
                                  assignment.id,
                                );
                              return (
                                ...
                                >
                                  <span className="text-sm text-slate-700">
                                    <strong>{fullName(assignment.user)}</strong>{" "}
                                  ...
                                  <form action={removeAssignment}>
                                    <ConfirmSubmitButton
                                      confirmation={`إزالة إسناد ${fullName(assignment.user)} من ${branch.name}؟`}
                                      type="submit"
                        ...
                        {branch.isActive && availableStaff.length > 0 ? (
                          ...
                          >
                            <select
                              ...
                              required
                              defaultValue=""
                              className={`${branchFieldClass} min-w-0 flex-1`}
                            >
                              <option value="" disabled>
                                اختر موظفًا نشطًا لإسناده
                              </option>
                          ...
                          <p className="mt-4 text-sm text-slate-500">
                            لا توجد حسابات موظفين نشطة متاحة للإسناد.
                          </p>
                        ) : (
                          <p className="mt-4 text-sm text-amber-700">
                            لا يمكن إنشاء إسنادات جديدة لفرع موقوف.
                          </p>
                        )}
                      </div>
                    </div>
...
function Notice({
  ...
  return (
    <div className={`mb-6 rounded-xl border px-4 py-3 ${className}`}>
      {children}
    </div>
  );
}
