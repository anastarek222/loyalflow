      ...
      <PageContainer variant="narrow" className="px-4 sm:px-6">
        ...
        {successMessage ? <Card role="status" className="border-success/30 bg-success-subtle p-6">
          ...
          <section aria-label={copy.balance}>
            ...
            <p dir={customer.business.loyaltyMode === "SALES_AMOUNT" ? "ltr" : "auto"} className="mt-1 lf-type-display lf-type-numeric text-foreground">{formatLoyaltyAmount({ ...loyaltyPresentation, amount: customer.balance })}</p>
          </section>
        ...
        <Card role="region" aria-label={copy.balance} className="border-primary/20 bg-primary-subtle/35 p-5 sm:p-6">
          ...
          <p dir={customer.business.loyaltyMode === "SALES_AMOUNT" ? "ltr" : "auto"} className="mt-2 lf-type-display lf-type-numeric font-black text-foreground">{formatLoyaltyAmount({ ...loyaltyPresentation, amount: customer.balance })}</p>
        </Card>
        ...
        {canEarn ? <Card className="p-6">
          ...
          <h2 className="mt-1 lf-type-section text-foreground">{earnActionLabel(loyaltyPresentation)}</h2>
          <form action={earnAction} className="mt-4">
            ...
            <ScanActionButton language={language}>{earnActionLabel(loyaltyPresentation)}</ScanActionButton>
          </form>
        </Card> : null}
        ...
        <details className="rounded-[var(--lf-radius-card)] border border-border bg-surface">
          ...
          {customer.transactions.length ? <div className="space-y-3 border-t border-border p-4 sm:p-5">{customer.transactions.map((transaction) => {
            ...
            return <Inset key={transaction.id} className={presentation.color}>
              ...
              <p dir={customer.business.loyaltyMode === "SALES_AMOUNT" ? "ltr" : "auto"} className="mt-1 font-semibold text-foreground">{formatLoyaltyAmount({ ...loyaltyPresentation, amount: transaction.amount })}</p>
