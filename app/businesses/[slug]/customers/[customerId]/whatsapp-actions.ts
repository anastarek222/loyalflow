...
function canSendManualWhatsApp(role: string) {
...
export async function sendManualCustomerWhatsAppAction(
  ...
  if (!parsed.success) redirect(customerPath(slug, customerId, "whatsapp-invalid"));
  ...
  if (
    ...
    !canSendManualWhatsApp(session.user.role)
  ) {
  ...
  if (
    ...
    redirect(customerPath(business.slug, customerId, "whatsapp-subscription-restricted"));
  }
  ...
  if (parsed.data.event === "REWARD_READY") {
    ...
    if (!availability.rewardReady) {
      redirect(customerPath(business.slug, customer.id, "whatsapp-reward-not-ready"));
    }
    rewardName = availability.affordableRewards[0]?.name ?? availability.defaultReward.name;
  }
