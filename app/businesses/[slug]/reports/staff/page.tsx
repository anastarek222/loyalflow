...
export default async function StaffReportsPage({
  ...
  for (const transaction of transactions) {
    ...
    const row =
      (creditedStaffId ? performance.get(creditedStaffId) : undefined) ??
      getSystemRow();
      ...
      >
        ...
        <p className="mt-3 text-sm text-slate-600" role="status">
          {reportScope.branchId
            ? `${t("سياق الفرع", "Branch context")}: ${reportBranches.find((branch) => branch.id === reportScope.branchId)?.name ?? "—"}`
            : t(
        ...
        <section className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
          {[
            [t("المستخدمون النشطون", "Active users"), activeUsers, "text-slate-950"],
            [t("إجمالي العمليات", "Total operations"), totalActions, "text-slate-950"],
            [t("الرصيد المضاف", "Earned balance"), totalEarned, "text-emerald-700"],
            [t("الرصيد المستبدل", "Redeemed balance"), totalRedeemed, "text-amber-700"],
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {rows.map((row) => (
                      <tr key={row.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-5 py-4 font-bold">{numberFormatter.format(row.customersCount)}</td>
                        <td className="px-5 py-4 font-bold">{numberFormatter.format(row.earnActions)}</td>
                        <td className="px-5 py-4 font-bold text-emerald-700">{numberFormatter.format(row.earnedAmount)}</td>
                        <td className="px-5 py-4 font-bold">{numberFormatter.format(row.redeemActions)}</td>
                        <td className="px-5 py-4 font-bold text-amber-700">{numberFormatter.format(row.redeemedAmount)}</td>
                        <td className="px-5 py-4 font-bold">{numberFormatter.format(row.rewardRedemptions)}</td>
                        <td className="px-5 py-4 font-bold">{numberFormatter.format(row.adjustmentActions)}</td>
                        <td className="px-5 py-4">
              ...
              {rows.map((row) => (
                <article key={row.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                    {[
                      [t("العملاء", "Customers"), row.customersCount, "bg-slate-50", "text-slate-500", ""],
                      [t("مكافآت مستبدلة", "Reward redemptions"), row.rewardRedemptions, "bg-amber-50", "text-amber-800", "text-amber-950"],
                      [t("الرصيد المضاف", "Earned balance"), row.earnedAmount, "bg-emerald-50", "text-emerald-700", "text-emerald-900"],
                      [t("الرصيد المستبدل", "Redeemed balance"), row.redeemedAmount, "bg-amber-50", "text-amber-700", "text-amber-900"],
                      [t("التعديلات", "Adjustments"), row.adjustmentActions, "bg-violet-50", "text-violet-700", "text-violet-900"],
                    ].map(([label, value, backgroundClass, labelClass, valueClass]) => (
                      <div key={String(label)} className={`rounded-xl p-3 ${backgroundClass}`}>
                        <p className={String(labelClass)}>{label}</p>
