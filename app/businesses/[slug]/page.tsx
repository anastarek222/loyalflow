...
function copy(language: "AR" | "EN") {
  return language === "AR"
    ? {
        eyebrow: "النظرة التشغيلية",
        overview: "ملخص اليوم",
        simpleReady: "جاهز للعميل التالي؟",
        simpleReadyDescription:
        ...
        allClearDescription: "لا توجد عناصر تشغيلية عاجلة الآن.",
        rewardReady: "عملاء بمكافأة جاهزة",
        ...
        activityToday: "حركة ولاء اليوم",
        redemptionsToday: "استبدال اليوم",
        newCustomers: "عملاء جدد هذا الشهر",
        totalCustomers: "إجمالي العملاء",
        trend: "نمو العملاء خلال 30 يومًا",
        trendDescription: "عرض سريع فقط — التحليل التفصيلي في التقارير.",
        noTrend: "لا توجد بيانات نمو كافية بعد.",
        segments: "اختصارات شرائح العملاء",
        segmentsDescription: "انتقل إلى فلتر مدعوم في صفحة العملاء.",
        activity: "آخر النشاط",
        activityDescription: "أحدث خمس حركات فقط.",
        viewActivity: "فتح سجل النشاط",
        quickActions: "إجراءات سريعة",
        quickActionsDescription: "أهم الإجراءات بدون البحث في القوائم.",
        growth: "فرص النمو",
        growthDescription: "انتقل إلى أدوات النمو الحالية عند الحاجة.",
        setupChecklist: "إكمال الإعداد",
        nextStep: "متابعة الإعداد",
        health: "صحة النشاط",
        team: "الفريق",
        branches: "الفروع",
        loyalty: "برنامج الولاء",
        salesLoyalty: "ولاء بالمبيعات",
        settings: "الإعدادات",
        loyaltyMode: "النظام",
        program: "البرنامج",
        active: "نشط",
        ...
        unit: "الوحدة",
        reward: "المكافأة",
        noActivity: "لا توجد حركات حديثة بعد.",
        openCustomers: "فتح العملاء",
        recovery: "استعادة العملاء",
        offers: "العروض",
        campaigns: "الحملات",
        rewards: "المكافآت",
    ...
    : {
        ...
        simpleReady: "Ready for the next customer?",
        simpleReadyDescription:
        ...
        allClear: "All clear",
        allClearDescription: "There are no urgent operational items right now.",
        rewardReady: "Customers with a reward ready",
        ...
        activityToday: "Loyalty activity today",
        redemptionsToday: "Redemptions today",
        newCustomers: "New customers this month",
        totalCustomers: "Total customers",
        trend: "Customer growth over 30 days",
        trendDescription:
          "A quick view only — use Reports for detailed analysis.",
        noTrend: "There is not enough growth data yet.",
        segments: "Customer segment shortcuts",
        segmentsDescription: "Open a supported filter in Customers.",
        activity: "Recent activity",
        activityDescription: "The latest five entries only.",
        viewActivity: "Open activity",
        quickActions: "Quick actions",
        quickActionsDescription:
          "The actions you use most, without hunting through menus.",
        growth: "Growth tools",
        growthDescription: "Use existing growth tools when they are needed.",
        setupChecklist: "Complete setup",
        nextStep: "Continue setup",
        health: "Business health",
        team: "Team",
        branches: "Branches",
        loyalty: "Loyalty programme",
        salesLoyalty: "Sales loyalty",
        settings: "Settings",
        loyaltyMode: "Mode",
        program: "Programme",
        active: "Active",
        ...
        unit: "Unit",
        reward: "Reward",
        noActivity: "There is no recent activity yet.",
...
export default async function BusinessPage({ params }: BusinessPageProps) {
  ...
  const actions = getBusinessDashboardActions(business.slug, {
    canScan,
    canViewReports,
    canManageSettings,
    canManageUsers,
  ...
  const primaryAdvancedActions = actions
    .filter((action) =>
      ["scan", "customers", "rewards", "offers", "reports"].includes(action.id),
    )
    .slice(0, 5);
  ...
  const [
    ...
    prisma.businessActivity.count({
      where: { businessId: business.id, type: "REWARD_REDEEMED" },
    }),
    ...
    prisma.businessActivity.count({
      where: { businessId: business.id, type: "BALANCE_ADJUSTED" },
    }),
    prisma.businessActivity.findMany({
      where: { businessId: business.id, type: "BALANCE_ADJUSTED" },
      orderBy: { createdAt: "desc" },
    ...
    prisma.businessActivity.count({
      where: { businessId: business.id, type: "LOYALTY_EARNED" },
    }),
    prisma.businessActivity.findMany({
      where: { businessId: business.id, type: "LOYALTY_EARNED" },
      orderBy: { createdAt: "desc" },
    ...
    canViewReports
      ? prisma.businessActivity.findMany({
          ...
          orderBy: { createdAt: "desc" },
          take: DASHBOARD_RECENT_ACTIVITY_LIMIT,
          select: {
            id: true,
            type: true,
            description: true,
  ...
  const rewardReadyCustomersWithReadState = rewardReadyCustomers.map(
    (customer) => ({
      ...customer,
  ...
  const onboarding = getBusinessOnboardingState({
          ...
          <PageHeader
            eyebrow={
              <span className="inline-flex items-center gap-2">
                <span
                  aria-hidden="true"
                  className="inline-flex size-6 items-center justify-center overflow-hidden rounded-full bg-surface-subtle text-[10px] font-bold text-foreground-muted"
                >
                  {business.logoUrl ? (
                    <img
            ...
            title={business.name}
            description={
              isSimpleExperience ? dictionary.simpleReady : dictionary.overview
            }
            status={
              !isSimpleExperience ? (
                <Badge variant={business.isActive ? "success" : "neutral"}>
                  {business.isActive ? dictionary.active : dictionary.inactive}
                </Badge>
              ) : null
            }
            metadata={
              isSimpleExperience ? (
                <>
                  ...
                  {business._count.branches > 1 ? (
                    <span>
                      {number.format(business._count.branches)}{" "}
                      {dictionary.branchContext}
                    </span>
                  ) : null}
                </>
              ) : (
                <>
                  <span>{roleLabel(user.role, language)}</span>
                  {businessContext ? (
                    <span dir="auto">{businessContext}</span>
                  ) : null}
                  <span>
                    {business.loyaltyMode === "SALES_AMOUNT"
                      ? dictionary.salesLoyalty
                      : `${dictionary.loyaltyMode}: ${business.unitName}`}
                  </span>
                  {business._count.branches > 1 ? (
                    <span>
                      {number.format(business._count.branches)}{" "}
            ...
            <section aria-label={dictionary.simpleReady}>
              <div className="grid gap-3 sm:grid-cols-2">
                {scanAction ? (
                  <Link
                    href={scanAction.href}
                    className="group flex min-h-28 items-center justify-between overflow-hidden rounded-[var(--lf-radius-card)] bg-gradient-to-br from-indigo-500 via-primary to-indigo-800 px-6 py-5 text-white shadow-lg shadow-primary/20 transition hover:shadow-xl sm:min-h-32"
                  >
                    <span>
                ...
                >
                  <span>
                    ...
                    <span className="mt-2 block text-xl font-black text-foreground sm:text-2xl">
                      {dictionary.findCustomer}
                    </span>
                    <span className="mt-1 block text-sm text-foreground-muted">
                      {dictionary.simpleReadyDescription}
                    </span>
                  </span>
            ...
            <section aria-label={dictionary.today}>
              <div className="grid grid-cols-3 overflow-hidden rounded-[var(--lf-radius-card)] border border-white/80 bg-white/85 shadow-sm">
                <div className="px-4 py-4 sm:px-5">
                  ...
                  <p
                    dir="ltr"
                    className="mt-1 lf-type-numeric text-2xl font-black text-foreground"
                  >
                    {number.format(todayActivity)}
                  </p>
                </div>
                <div className="border-s border-border px-4 py-4 sm:px-5">
                  <p className="text-xs font-semibold text-foreground-subtle">
                    {dictionary.redemptionsToday}
                  </p>
                  <p
                    dir="ltr"
                    className="mt-1 lf-type-numeric text-2xl font-black text-foreground"
                  >
                    {number.format(todayRedemptions)}
                  </p>
                </div>
                <div className="border-s border-border px-4 py-4 sm:px-5">
                  <p className="text-xs font-semibold text-foreground-subtle">
                  ...
                  <p
                    ...
                    className="mt-1 lf-type-numeric text-2xl font-black text-foreground"
                  >
                    {number.format(rewardReadyCount)}
                  </p>
                </div>
              </div>
            </section>
                    ...
                    {operationalAlerts.slice(0, 3).map((alert) =>
                      alert.href ? (
                        <Link
                          key={alert.id}
                          href={alert.href}
                          className="flex min-h-12 items-center justify-between gap-3 py-3 text-sm font-semibold text-foreground hover:text-primary"
                        >
                        ...
                        <div
                          key={alert.id}
                          className="flex min-h-12 items-center justify-between gap-3 py-3 text-sm font-semibold text-foreground"
                        >
                          <span>{alert.label}</span>
                          {alert.count ? (
                            <span
                              dir="ltr"
                              className="lf-type-numeric text-warning"
                            >
                              {number.format(alert.count)}
                            </span>
                          ) : null}
                  ...
                  <div className="mt-2 divide-y divide-border">
                    {recentActivities.slice(0, 3).map((activity) => (
                      ...
                      >
                        ...
                        >
                          {date.format(activity.createdAt)}
                        </time>
                      </article>
                    ))}
                  </div>
          ...
          <div data-experience-mode={experienceMode} className="space-y-6">
              ...
              >
                <SectionHeader
                  ...
                  description={
                    operationalAlerts.length
                      ? undefined
                      : dictionary.allClearDescription
                  }
                />
                {operationalAlerts.length ? (
                    ...
                    {operationalAlerts.map((alert) =>
                      alert.href ? (
                        <Link
                          key={alert.id}
                          href={alert.href}
                          className="flex min-h-12 items-center justify-between rounded-[var(--lf-radius-input)] border border-border bg-surface-subtle px-4 text-sm font-semibold text-foreground hover:border-primary"
                        >
                          <span>{alert.label}</span>
                          <span className="text-primary">
                            {alert.count ? number.format(alert.count) : null}
                          </span>
                        </Link>
                      ) : (
                        <div
                          key={alert.id}
                          className="rounded-[var(--lf-radius-input)] border border-border bg-surface-subtle p-4 text-sm font-semibold text-foreground"
                        >
                          {alert.label}
                        </div>
                      ),
            ...
            <section aria-label="Daily key performance indicators">
              <StatGrid>
                <StatCard
                  ...
                  value={number.format(business._count.customers)}
                  supportingText={`${number.format(activeCustomers)} ${dictionary.activeCustomers}`}
                  status="info"
                  icon={<Users size={18} />}
                />
                <StatCard
                  label={dictionary.activityToday}
                  value={number.format(todayActivity)}
                  supportingText={dictionary.activityLabel}
                  status="neutral"
                />
                <StatCard
                  label={dictionary.redemptionsToday}
                  value={number.format(todayRedemptions)}
                  supportingText={dictionary.reward}
                  status="success"
                />
                <StatCard
                  label={dictionary.newCustomers}
                  value={number.format(newCustomersMonth)}
                  supportingText={dictionary.customers}
                  status="info"
                />
              </StatGrid>
            </section>
                ...
                <Card className="border-white/80 bg-white/85">
                  ...
                  <div className="mt-4">
                    <DashboardHealthChart
                      data={customerGrowth}
                      emptyLabel={dictionary.noTrend}
                      summary={dictionary.trend}
                    />
                  </div>
                </Card>
                <Card className="border-white/80 bg-white/85">
                  ...
                  {recentActivities.length ? (
                    <div className="mt-4 divide-y divide-border">
                      {recentActivities.map((activity) => (
                        <article
                          key={activity.id}
                          className="flex items-start justify-between gap-4 rounded-xl px-2 py-4 transition hover:bg-surface-subtle/70 first:pt-2 last:pb-2"
                        >
                          <div className="min-w-0">
            ...
            {showOnboarding ? (
              <section>
                <Card>
                  <SectionHeader
                    title={dictionary.setupChecklist}
                    description={dictionary.setupDescription}
                    actions={
                      <Link
                  ...
                  <Progress
                    ...
                    label={dictionary.setupChecklist}
                  />
                  <div className="mt-4 flex flex-wrap gap-2 text-sm text-foreground-muted">
                    <Badge
                      variant={onboarding.teamComplete ? "success" : "warning"}
                    >
                    ...
                    <Badge
                      variant={
                        onboarding.loyaltyComplete ? "success" : "warning"
                      }
                    >
            ...
            {canManageSettings || canManageUsers ? (
              <section>
                <Card>
                  <SectionHeader title={dictionary.health} />
                  <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                    {canManageUsers ? (
                      <Link
                        href={`/businesses/${business.slug}/users`}
                        className="rounded-[var(--lf-radius-input)] border border-border p-4 hover:border-primary"
                      >
                        ...
                        <p
                          dir="ltr"
                          className="mt-1 lf-type-numeric text-foreground-muted"
                        >
                          {number.format(business._count.users)}
                        </p>
                      </Link>
                    ) : null}
                    {canManageSettings ? (
