    ...
    <>
      ...
      >
        <div data-experience-mode={experienceMode} className={isSimpleExperience ? "space-y-6 [&>section:nth-of-type(4)]:hidden [&>section:nth-of-type(6)]:hidden [&>section:nth-of-type(7)]:hidden [&>section:nth-of-type(8)]:hidden" : "space-y-6"}>
          <section aria-label={operationalAlerts.length ? dictionary.needsAttention : dictionary.allClear}><Card className={operationalAlerts.length ? "border-warning/30" : ""}><SectionHeader title={operationalAlerts.length ? dictionary.needsAttention : dictionary.allClear} description={operationalAlerts.length ? undefined : dictionary.allClearDescription} /><div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{operationalAlerts.map((alert) => alert.href ? <Link key={alert.id} href={alert.href} className="rounded-[var(--lf-radius-input)] border border-border bg-surface-subtle p-4 text-sm font-semibold text-foreground-muted hover:border-primary">{alert.count ? <span dir="ltr" className="me-2 lf-type-numeric">{number.format(alert.count)}</span> : null}{alert.label}<ArrowUpRight className="ms-2 inline" size={15} aria-hidden="true" /></Link> : <div key={alert.id} className="rounded-[var(--lf-radius-input)] border border-border bg-surface-subtle p-4 text-sm font-semibold text-foreground-muted">{alert.count ? <span dir="ltr" className="me-2 lf-type-numeric">{number.format(alert.count)}</span> : null}{alert.label}{alert.id === "unread" ? <p className="mt-1 text-xs font-normal text-foreground-muted">{dictionary.useNotifications}</p> : null}</div>)}</div></Card></section>
