...
const settingsSchema = z.object({
  ...
  whatsappWelcomeMessage: z
    .string()
    .trim()
    .min(1)
    .max(1500),

  whatsappBalanceMessage: z
    .string()
    .trim()
    .min(1)
    .max(1500),

  whatsappRewardMessage: z
    .string()
    .trim()
    .min(1)
    .max(1500),
...
export async function updateBusinessSettingsAction(
  ...
  if (
    ...
    if (
      ...
      redirect(
        `/businesses/${business.slug}/settings?error=invalid`
      );
    }
  ...
  const parsed = settingsSchema.safeParse({
    ...
    whatsappWelcomeMessage:
      formData.get("whatsappWelcomeMessage"),
    whatsappBalanceMessage:
      formData.get("whatsappBalanceMessage"),
    whatsappRewardMessage:
      formData.get("whatsappRewardMessage"),
  });
  ...
  await prisma.$transaction([
    prisma.business.update({
      ...
      data: {
        ...
        rewardCode:
          parsed.data.rewardCode || null,
        rewardDescription:
          parsed.data.rewardDescription || null,
        rewardThreshold: parsed.data.rewardThreshold,
        earnAmount: parsed.data.earnAmount,
        whatsappWelcomeMessage:
          parsed.data.whatsappWelcomeMessage,
        whatsappBalanceMessage:
          parsed.data.whatsappBalanceMessage,
        whatsappRewardMessage:
          parsed.data.whatsappRewardMessage,
      },
    }),
  ...
  revalidatePath(
    `/businesses/${business.slug}/activity`
  );
  ...
  redirect(
    `/businesses/${business.slug}/settings?saved=1`
  );
}
...
export async function updateBusinessCardDetailsAction(
  ...
  const parsed =
    cardBusinessDetailsSchema.safeParse({
      contactPhone:
        formData.get("contactPhone"),
      address:
        formData.get("address"),
      cardTerms:
        formData.get("cardTerms"),
    });

  if (!parsed.success) {
    redirect(
      `/businesses/${business.slug}/settings?cardError=invalid`
    );
  }
  ...
  redirect(
    `/businesses/${business.slug}/settings?cardSaved=1`
  );
}
...
export async function updateBusinessExportPermissionAction(
  ...
  redirect(
    `/businesses/${business.slug}/settings?exportPermissionSaved=1`
  );
}
