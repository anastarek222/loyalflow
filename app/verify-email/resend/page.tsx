...
export default async function ResendEmailVerificationPage({
  ...
  const [{ sent }, locale] = await Promise.all([searchParams, getAuthEntryLocale()]);
    ...
    <main lang={locale} dir={direction} className="min-h-screen bg-surface-subtle px-4 py-12">
      <section className="mx-auto w-full max-w-md rounded-[var(--lf-radius-card)] border border-border bg-surface p-6 sm:p-8">
        ...
        <form action={resendEmailVerificationAction} className="mt-6 space-y-4">
          ...
          />
          <button type="submit" className="min-h-11 w-full rounded-[var(--lf-radius-input)] bg-primary px-4 py-3 font-semibold text-white transition hover:bg-primary-hover">
