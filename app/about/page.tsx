...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = translate(locale, "about.metaTitle");
  const description = translate(locale, "about.metaDescription");
...
export default async function AboutPage() {
  ...
  const principles = [
    [Users, "about.customerTitle", "about.customerBody"],
    [ScanLine, "about.teamTitle", "about.teamBody"],
    [Store, "about.businessTitle", "about.businessBody"],
  ] as const;
  return (
    <main lang={locale} dir={direction} className="min-h-screen bg-[var(--lf-marketing-canvas)] text-foreground">
      ...
      <section className="border-b border-border bg-[linear-gradient(180deg,#ffffff,#f5f7ff)] px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
        <div className="mx-auto w-full max-w-7xl">
          <p className="text-sm font-black uppercase tracking-[0.16em] text-primary">{copy("about.eyebrow")}</p>
          <h1 className="mt-4 max-w-5xl text-4xl font-black tracking-tight sm:text-5xl lg:text-6xl">{copy("about.title")}</h1>
          <p className="mt-6 max-w-3xl text-lg leading-8 text-foreground-muted">{copy("about.body")}</p>
        </div>
      </section>
      <section className="px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        ...
        <div className="mx-auto mt-10 flex w-full max-w-7xl flex-col gap-6 rounded-[1.5rem] bg-slate-950 p-7 text-white sm:p-10 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-3xl"><h2 className="text-2xl font-black">{copy("about.ctaTitle")}</h2><p className="mt-3 leading-7 text-slate-300">{copy("about.ctaBody")}</p></div>
