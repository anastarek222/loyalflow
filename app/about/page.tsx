...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    ...buildPublicSocialMetadata({ title, description, path: "/about" }),
  };
}
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        <div className="mx-auto grid w-full max-w-7xl gap-5 md:grid-cols-3">
          {principles.map(([Icon, titleKey, bodyKey]) => (
            <article
              key={titleKey}
              className="rounded-[1.35rem] border border-border bg-white p-7 transition-colors hover:border-primary/35"
            >
        ...
        <div className="mx-auto mt-10 flex w-full max-w-7xl flex-col gap-6 rounded-[1.5rem] bg-foreground p-7 text-white sm:p-10 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-3xl">
            <h2 className="text-2xl font-black">
              {copy("marketing.about.ctaTitle")}
            </h2>
            <p className="mt-3 leading-7 text-white/75">
              {copy("marketing.about.ctaBody")}
            </p>
          </div>
          <Link
            href="/get-started"
            className="inline-flex min-h-12 shrink-0 items-center justify-center rounded-xl bg-white px-5 py-3 font-bold text-foreground transition-colors hover:bg-[var(--lf-primary-soft)]"
          >
