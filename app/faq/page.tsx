      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        <div className="mx-auto w-full max-w-4xl divide-y divide-border border-y border-border bg-white px-5 sm:px-8">
          {questions.map(([questionKey, answerKey], index) => (
            <details key={questionKey} className="group py-3" open={index === 0}>
              <summary className="flex min-h-16 cursor-pointer list-none items-center justify-between gap-4 py-3 text-lg font-black marker:content-none">
                <span>{copy(questionKey)}</span><span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary transition-transform group-open:rotate-45" aria-hidden="true">+</span>
              </summary>
              <p className="max-w-3xl pb-6 text-sm leading-7 text-foreground-muted">{copy(answerKey)}</p>
            </details>
          ))}
        </div>
      </section>
