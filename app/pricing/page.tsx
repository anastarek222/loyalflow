  ...
  return (
    ...
    >
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        <div className="mx-auto grid w-full max-w-7xl gap-5 md:grid-cols-2 xl:grid-cols-4">
          {loyalFlowPlans.map((plan) => {
            ...
            return (
              <article
                key={plan}
                className={`flex flex-col rounded-[1.35rem] border bg-white p-6 ${isFree ? "border-primary ring-2 ring-primary/10" : "border-border"}`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.12em] text-primary">
                      {isFree
                        ? copy("marketing.pricing.freeLabel")
                        : copy("marketing.pricing.betaLabel")}
                    </p>
                ...
                <p className="mt-4 min-h-12 text-sm font-semibold leading-6 text-foreground-muted">
                  {isFree
                    ? copy("marketing.pricing.freeLabel")
                    : copy("marketing.pricing.confirmedAtSetup")}
                </p>
                      ...
                      >
                        ...
                        <span>
                          <strong className="text-foreground">
                            {limit === null
                              ? copy("marketing.pricing.unlimited")
                              : limit.toLocaleString(locale)}
                          </strong>{" "}
                          {
                            resourceLabels[
                              resource as keyof typeof resourceLabels
                            ]
                          }
                        </span>
                      </li>
                ...
                <Link
                  href="/get-started"
                  className={`mt-7 inline-flex min-h-11 items-center justify-center rounded-xl px-4 text-sm font-bold ${isFree ? "bg-primary text-white" : "border border-border bg-surface-subtle text-foreground"}`}
                >
        ...
        <div className="mx-auto mt-10 w-full max-w-7xl rounded-[1.35rem] border border-primary/25 bg-[var(--lf-primary-soft)] p-6 sm:p-8">
          <h2 className="text-xl font-black text-foreground">
            {copy("marketing.pricing.noteTitle")}
          </h2>
          <p className="mt-3 max-w-4xl leading-7 text-foreground-muted">
            {copy("marketing.pricing.noteBody")}
          </p>
        </div>
      </section>
