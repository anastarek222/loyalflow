...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = translate(locale, "pricing.metaTitle");
  const description = translate(locale, "pricing.metaDescription");
  return {
    title,
    description,
...
export default async function PricingPage() {
  ...
  const resourceLabels = {
    CUSTOMERS: copy("pricing.customers"),
    USERS: copy("pricing.users"),
    BRANCHES: copy("pricing.branches"),
    OFFERS: copy("pricing.offers"),
    REWARDS: copy("pricing.rewards"),
  } as const;
    ...
    >
      ...
      <section className="border-b border-border bg-white px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
        <div className="mx-auto w-full max-w-7xl text-center">
          <p className="text-sm font-black uppercase tracking-[0.16em] text-primary">{copy("pricing.eyebrow")}</p>
          <h1 className="mx-auto mt-4 max-w-4xl text-4xl font-black tracking-tight sm:text-5xl lg:text-6xl">{copy("pricing.title")}</h1>
          <p className="mx-auto mt-6 max-w-3xl text-lg leading-8 text-foreground-muted">{copy("pricing.body")}</p>
        </div>
      </section>
            ...
            return (
              <article key={plan} className={`flex flex-col rounded-[1.35rem] border bg-white p-6 shadow-[0_18px_45px_rgb(15_23_42/0.06)] ${isFree ? "border-primary ring-2 ring-primary/10" : "border-border"}`}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.12em] text-primary">{isFree ? copy("pricing.freeLabel") : copy("pricing.betaLabel")}</p>
                ...
                <p className="mt-4 min-h-12 text-sm font-semibold leading-6 text-foreground-muted">{isFree ? copy("pricing.freeLabel") : copy("pricing.confirmedAtSetup")}</p>
                <ul className="mt-6 flex-1 space-y-3 border-t border-border pt-5">
                  {pricedResources.map((resource) => {
                    ...
                    <li key={resource} className="flex items-center gap-2 text-sm text-foreground-muted">
                      ...
                      <span><strong className="text-foreground">{limit === null ? copy("pricing.unlimited") : limit.toLocaleString(locale)}</strong> {resourceLabels[resource as keyof typeof resourceLabels]}</span>
                    </li>
                    );
                  })}
                </ul>
                <Link href="/get-started" className={`mt-7 inline-flex min-h-11 items-center justify-center rounded-xl px-4 text-sm font-bold ${isFree ? "bg-primary text-white" : "border border-border bg-surface-subtle text-foreground"}`}>{copy("pricing.cta")}</Link>
              </article>
            );
          })}
        </div>
        <div className="mx-auto mt-10 w-full max-w-7xl rounded-[1.35rem] border border-indigo-200 bg-indigo-50 p-6 sm:p-8">
          <h2 className="text-xl font-black text-indigo-950">{copy("pricing.noteTitle")}</h2>
          <p className="mt-3 max-w-4xl leading-7 text-indigo-900/75">{copy("pricing.noteBody")}</p>
        </div>
      </section>
