...
- PR #175 is actively wired: Business Settings manual Google Sheets sync uses `syncGoogleSheetCommandAction`. The command rechecks persisted `OPERATE` immediately before delegating to the existing safe sync helper, while provider interaction and sync-state persistence remain owned by `syncBusinessToGoogleSheetSafely`. Final-head run #172 passed focused tests, the full suite, typecheck, workspace validation, lint, Next build, and patch whitespace. Exact-head Vercel is blocked by Hobby `build-rate-limit`.
...
3. Manual Google Sheets sync (#175) is now actively command-backed; provider configuration and sync-state recording remain unchanged.
4. Continue TC5 by auditing the remaining operational write surfaces that are not yet represented by an actively wired Draft; keep one bounded surface per PR.
5. Do not introduce `/api/v1` write Route Handlers for current UI consumption. Server Actions remain the approved compatibility transport under the TC5 safe-write policy.
6. Runtime/TCR evidence remains distinct from TC/TR and must be collected only when a fresh Staging/Preview runtime is available.
7. Vercel Hobby build-rate-limit remains an external runtime-evidence blocker for several exact heads; do not reinterpret GitHub TC/TR success as browser TCR completion.
