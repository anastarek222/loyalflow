...
TC7 Beta acquisition foundation is complete for invitation-only operation. The measured disposable-database backup/restore rehearsal remains the next recovery item, but it is blocked until tooling can create a temporary branch explicitly from isolated Staging; it must never clone or mutate Production. Other next slices must come from the remaining registered domains without bypassing their named decisions.
