...
| TC6 integrations   | health aggregation, retry-eligibility contracts, a privacy-minimized read-only Operations status snapshot, and an additive business-scoped outbox schema with idempotent enqueue plus claim/lease/result persistence primitives | transactional source enqueue, worker trigger/consumer, retry execution/backoff, pending aging thresholds, SLO/severity/alerts | approved Staging migration, execution topology, runtime replay/duplicate-prevention and recovery evidence |
