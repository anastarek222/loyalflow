...
| TC4 subscriptions  | provider-neutral lifecycle/access policy, read-only Operations projection, and additive lifecycle persistence with truthful legacy backfill, optimistic concurrency, Super Admin transitions, and audit evidence | runtime entitlement enforcement, trusted provider events, checkout, billing and payment activation                                            | isolated migration/runtime UAT, trusted idempotent event contract, provider credentials, and payment-flow evidence |
