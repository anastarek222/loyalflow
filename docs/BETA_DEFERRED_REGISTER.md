...
| TC4 subscriptions  | provider-neutral lifecycle/access policy, read-only Operations projection, additive lifecycle persistence, and runtime enforcement for loyalty operations plus customer, branch, and team expansion                 | offers, rewards, campaigns, remaining operational/economic write parity, trusted provider events, checkout, billing and payment activation    | parity evidence, trusted idempotent event contract, provider credentials, and payment-flow evidence   |
