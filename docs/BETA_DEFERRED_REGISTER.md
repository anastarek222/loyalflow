...
| TC4 subscriptions  | provider-neutral lifecycle/access policy, read-only Operations projection, additive lifecycle persistence, and runtime enforcement for core loyalty operations, financial reversals, customer/team/resource expansion, resource maintenance, workforce topology, customer profile/note maintenance, and individual/bulk customer reactivation | referral/tag topology, settings, and remaining admin write parity; trusted provider events and campaign execution; checkout and billing activation | parity evidence, trusted idempotent event contract, provider credentials, and payment-flow evidence   |
