...
| TC4 subscriptions  | provider-neutral lifecycle/access policy, read-only Operations projection, additive lifecycle persistence, and runtime enforcement for core loyalty operations, financial reversals, customer/team/resource expansion, resource maintenance, workforce topology, customer profile/note maintenance, customer reactivation, and bulk customer-tag topology | referral identity, individual tag creation/assignment, settings, and remaining admin write parity; trusted provider events and campaign execution; checkout and billing activation | parity evidence, trusted idempotent event contract, provider credentials, and payment-flow evidence   |
