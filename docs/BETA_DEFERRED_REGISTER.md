...
| TC4 subscriptions  | provider-neutral lifecycle/access policy, read-only Operations projection, additive lifecycle persistence, and initial runtime enforcement for loyalty operations and customer expansion                         | route-wide write/expansion parity, trusted provider events, checkout, billing and payment activation                                          | parity evidence, trusted idempotent event contract, provider credentials, and payment-flow evidence   |
