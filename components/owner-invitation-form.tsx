...
export function OwnerInvitationForm({ action }: { action: (formData: FormData) => void | Promise<void> }) {
  ...
  return <form action={action} onSubmit={(event) => { const parsed = ownerInvitationSchema.safeParse(Object.fromEntries(new FormData(event.currentTarget))); if (!parsed.success) { event.preventDefault(); const field = String(parsed.error.issues[0]?.path[0] ?? "details"); setError(`Please check ${field === "ownerPassword" ? "the temporary password" : field === "ownerEmail" ? "the owner email address" : "the owner details"}.`); } }} className="mt-5 space-y-4">
    ...
    <input name="ownerPassword" required type="password" minLength={MIN_PASSWORD_LENGTH} maxLength={100} autoComplete="new-password" placeholder={`Temporary password (minimum ${MIN_PASSWORD_LENGTH} characters)`} className="w-full rounded-xl border px-4 py-3" />
    <button type="submit" className="w-full rounded-xl bg-primary px-4 py-3 font-semibold text-white hover:bg-primary-hover">Create owner invitation</button>
  </form>;
}
