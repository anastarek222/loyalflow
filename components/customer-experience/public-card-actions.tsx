...
export function PublicCardActions({ cardUrl, businessName, customerName, language }: Props) {
  ...
  return <section className="mt-3" aria-label={copy.share}>
    <div className={`grid grid-cols-2 gap-3 ${canShowInstall ? "sm:grid-cols-3" : ""}`}>
      <button type="button" aria-label={copy.share} onClick={share} className="min-h-11 rounded-xl border border-white/15 bg-white/10 px-4 py-3 text-sm font-black text-white backdrop-blur transition hover:bg-white/15 focus:outline-none focus:ring-2 focus:ring-white">{copy.share}</button>
      <button type="button" aria-label={copy.copyLink} onClick={copyLink} className="min-h-11 rounded-xl border border-white/15 bg-white/10 px-4 py-3 text-sm font-black text-white backdrop-blur transition hover:bg-white/15 focus:outline-none focus:ring-2 focus:ring-white">{copy.copyLink}</button>
      {canShowInstall ? <button type="button" aria-label={installed ? copy.installed : copy.install} onClick={install} disabled={installed} className="col-span-2 min-h-11 rounded-xl border border-white/15 bg-white/10 px-4 py-3 text-sm font-black text-white backdrop-blur transition hover:bg-white/15 focus:outline-none focus:ring-2 focus:ring-white disabled:cursor-default disabled:opacity-70 sm:col-span-1">{installed ? copy.installed : copy.install}</button> : null}
    </div>
    {notice ? <p role="status" aria-live="polite" className="mt-3 text-center text-xs font-bold text-white/85">{notice}</p> : null}
