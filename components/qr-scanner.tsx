...
export default function QrScanner({ businessId, language }: QrScannerProps) {
  ...
  return <div>
    ...
    <div className="my-8 flex items-center gap-4"><div className="h-px flex-1 bg-surface-subtle" /><span className="text-xs font-semibold uppercase text-foreground-subtle">{copy.manualDivider}</span><div className="h-px flex-1 bg-surface-subtle" /></div>
    <form onSubmit={submitManualValue} className="space-y-4"><label htmlFor="manualQrValue" className="block text-sm font-medium text-foreground-muted">{copy.manualLabel}</label><input id="manualQrValue" value={manualValue} onChange={(event) => setManualValue(event.target.value)} placeholder={copy.manualPlaceholder} dir="ltr" className="min-h-11 w-full rounded-[var(--lf-radius-input)] border border-border bg-white px-4 text-black placeholder:text-foreground-subtle outline-none focus:border-primary/30 focus:ring-4 focus:ring-primary/20" /><button type="submit" disabled={!manualValue.trim() || isProcessing} aria-disabled={!manualValue.trim() || isProcessing} aria-busy={isProcessing} className="min-h-11 w-full rounded-[var(--lf-radius-input)] bg-foreground px-6 py-4 font-semibold text-white transition hover:bg-primary-subtle disabled:cursor-not-allowed disabled:bg-surface-subtle disabled:text-foreground-subtle">{isProcessing ? copy.processing : copy.openCustomer}</button></form>
  </div>;
}
