...
export function EmptyState({ title, description, icon, action, className }: { title: ReactNode; description: ReactNode; icon?: ReactNode; action?: ButtonProps; className?: string }) { return <section className={cn("grid min-h-48 place-items-center rounded-lg border border-dashed border-border-strong bg-surface-subtle p-6 text-center", className)}><div className="max-w-sm">{icon && <div aria-hidden="true" className="mx-auto mb-3 text-slate-500">{icon}</div>}<h2 className="lf-type-card text-slate-900">{title}</h2><p className="mt-1 text-sm leading-6 text-slate-600">{description}</p>{action && <Button {...action} className={cn("mt-4", action.className)} />}</div></section>; }
