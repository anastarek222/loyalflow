...
export function Toolbar({ className, ...props }: HTMLAttributes<HTMLDivElement>) { return <div {...props} className={cn("flex flex-col gap-4 rounded-[var(--lf-radius-input)] border border-border bg-surface p-4 sm:flex-row sm:items-center sm:justify-between", className)} />; }
...
export function FilterChip({ children, onRemove }: { children: ReactNode; onRemove?: () => void }) { return <Badge variant="neutral">{children}{onRemove && <button type="button" onClick={onRemove} aria-label={`Remove ${typeof children === "string" ? children : "filter"}`} className="ms-1 min-h-0 leading-none text-foreground-subtle hover:text-foreground">×</button>}</Badge>; }
