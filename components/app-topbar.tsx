...
export default function AppTopbar({ language, experienceMode, experienceAccess, user, businesses, activeBusiness }: Props) {
  ...
  return <header className="lf-topbar sticky top-0 z-20 flex min-h-16 items-center justify-between gap-4 border-b px-4 py-2 sm:px-6" data-shell-topbar="true">
    <div className="flex min-w-0 items-center gap-2 sm:gap-4">
      ...
      {activeBusiness && <div ref={businessRef} className="relative hidden sm:block" data-current-business-context="true">
        {businesses.length > 1 ? <button type="button" aria-expanded={businessOpen} aria-haspopup="menu" onClick={() => setBusinessOpen((value) => !value)} className="flex min-h-11 max-w-52 items-center gap-2 rounded-[var(--lf-radius-input)] border border-border px-4 text-sm font-semibold text-foreground-muted hover:bg-surface-subtle"><Building2 size={16} aria-hidden="true" /><span className="truncate">{activeBusiness.name}</span><ChevronDown size={15} aria-hidden="true" /></button> : <span className="inline-flex min-h-11 max-w-52 items-center gap-2 rounded-[var(--lf-radius-input)] border border-border px-4 text-sm font-semibold text-foreground-muted"><Building2 size={16} aria-hidden="true" /><span className="truncate">{activeBusiness.name}</span></span>}
