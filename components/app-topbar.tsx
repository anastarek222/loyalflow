...
import { Bell, Building2, ChevronDown, KeyRound, LogOut, ScanLine } from "lucide-react";
  ...
  return <header className="lf-topbar sticky top-0 z-20 flex min-h-14 items-center justify-between gap-2 border-b px-3 py-2 sm:gap-4 sm:px-6" data-shell-topbar="true">
    <div className="flex min-w-0 flex-1 items-center gap-1 sm:gap-4">
      ...
      {activeBusiness && businesses.length > 1 && <div ref={businessRef} className="relative hidden sm:block" data-current-business-context="true">
        <button type="button" aria-expanded={businessOpen} aria-haspopup="menu" onClick={() => setBusinessOpen((value) => !value)} className="flex min-h-10 max-w-52 items-center gap-2 rounded-[var(--lf-radius-input)] border border-border px-3 text-sm font-semibold text-foreground-muted hover:bg-surface-subtle"><Building2 size={16} aria-hidden="true" /><span className="truncate">{activeBusiness.name}</span><ChevronDown size={15} aria-hidden="true" /></button>
    ...
    <div className="flex shrink-0 items-center gap-0 sm:gap-2">
      ...
      <div ref={accountRef} className="relative">
        <button type="button" aria-expanded={accountOpen} aria-haspopup="menu" aria-label={language === "AR" ? "قائمة الحساب" : "Account menu"} onClick={() => setAccountOpen((value) => !value)} className="flex min-h-11 items-center gap-2 rounded-[var(--lf-radius-input)] px-1.5 hover:bg-surface-subtle"><Avatar name={fullName || user.email} className="bg-foreground text-white" /><ChevronDown className="hidden text-foreground-subtle sm:block" size={16} aria-hidden="true" /></button>
        {accountOpen && <div role="menu" aria-label={language === "AR" ? "الحساب" : "Account"} className="absolute end-0 top-12 z-50 w-72 rounded-[var(--lf-radius-input)] border border-border bg-surface p-2 shadow-[var(--lf-shadow-overlay)]"><div className="border-b border-border px-4 py-2"><p className="font-semibold text-foreground">{fullName || "User"}</p><p dir="ltr" className="mt-0.5 truncate text-sm text-foreground-subtle">{user.email}</p><p className="mt-1 text-xs font-semibold text-foreground-muted">{roleLabel(user.role, language)}</p></div>{modeRules.showModeSwitcher ? <ExperienceModeSwitcher language={language} mode={experienceMode} access={experienceAccess} /> : null}<div className="pt-1"><Link href="/account/security" role="menuitem" onClick={() => setAccountOpen(false)} className="flex min-h-11 items-center gap-2 rounded-[var(--lf-radius-input)] px-4 text-sm font-semibold text-foreground-muted hover:bg-surface-subtle"><KeyRound size={16} aria-hidden="true" />{language === "AR" ? "أمان الحساب" : "Account security"}</Link>{user.role === "SUPER_ADMIN" && <><Link href="/businesses" role="menuitem" onClick={() => setAccountOpen(false)} className="flex min-h-11 items-center gap-2 rounded-[var(--lf-radius-input)] px-4 text-sm font-semibold text-foreground-muted hover:bg-surface-subtle"><Building2 size={16} aria-hidden="true" />{language === "AR" ? "الأنشطة التجارية" : "Businesses"}</Link><Link href="/business-owners" role="menuitem" onClick={() => setAccountOpen(false)} className="flex min-h-11 items-center gap-2 rounded-[var(--lf-radius-input)] px-4 text-sm font-semibold text-foreground-muted hover:bg-surface-subtle"><Building2 size={16} aria-hidden="true" />{language === "AR" ? "ملاك الأنشطة" : "Business owners"}</Link></>}<form action={logoutAction}><button type="submit" role="menuitem" className="flex min-h-11 w-full items-center gap-2 rounded-[var(--lf-radius-input)] px-4 text-sm font-semibold text-danger hover:bg-surface-subtle"><LogOut size={16} aria-hidden="true" />{language === "AR" ? "تسجيل الخروج" : "Log out"}</button></form></div></div>}
      </div>
    </div>
  </header>;
}
