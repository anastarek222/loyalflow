  ...
  return (
    <aside className="lf-nav-sidebar sticky top-0 hidden h-screen w-72 shrink-0 border-e lg:flex lg:flex-col" aria-label={language === "AR" ? "التنقل الرئيسي" : "Primary navigation"}>
      <div className="border-b border-border px-6 py-6">
        <Link href="/dashboard" className="flex items-center gap-4 rounded-[var(--lf-radius-input)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--lf-focus)]">
          <span className="flex size-10 items-center justify-center rounded-[var(--lf-radius-input)] bg-foreground text-lg font-black text-white">L</span>
          <span><span className="block text-lg font-black text-foreground">LoyalFlow</span><span className="block text-xs font-medium text-foreground-subtle">Loyalty operations</span></span>
        </Link>
      </div>
      {business && (
        <div className="border-b border-border px-6 py-4" data-current-business-context="true">
        ...
        {groups.map((group) => <section key={group.id} className="mb-6 last:mb-0">
          ...
          <ul className="space-y-1">
            {group.items.map((entry) => {
              ...
              return <li key={entry.href}><Link href={entry.href} aria-current={active ? "page" : undefined} className={`lf-nav-item flex min-h-11 items-center gap-4 px-4 text-sm font-semibold transition-colors ${active ? "lf-nav-item-active" : ""}`}><Icon size={18} aria-hidden="true" /><span>{entry.label}</span></Link></li>;
            })}
          </ul>
        </section>)}
      </nav>
      <div className="border-t border-border px-6 py-4 text-xs text-foreground-subtle">LoyalFlow</div>
    </aside>
  );
}
